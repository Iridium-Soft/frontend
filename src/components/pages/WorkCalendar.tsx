...
export default class WorkCalendar extends Component<Props, State> {
  ...
  handleAdd() {
    if (!/[a-zA-Z]+/.test(this.state.nombre)) {
      this.setState({
        message:
          "El siguiente campo debe ser correctamente llenado: Nombre incorrecto",
        open: true,
      });
      return;
    } else if (
      ...
      this.setState({
        message:
          "El siguiente campo debe ser correctamente llenado: Fechas incorrectas",
        open: true,
      });
      return;
    } else if (
      this.state.porcentajeCobro + this.state.sum > 100 ||
      this.state.sum == 100
    ) {
      this.setState({
        message:
          "Llene correctamente los campos. Sumatoria de porcentaje sobrepasa 100%",
        open: true,
      });
      return;
    }
        ...
        <div className="container p-3 position-relative">
            ...
            <div className="col col-12">
              <table className="table table-bordered">
                <tbody>
                  ...
                  {milestones &&
                    milestones.hitos.map((hi: any) => (
                      <>
                        <tr>
                          <td>{hi.nombre}</td>
                          <td>{hi.fechaIni}</td>
                          <td>{hi.fechaFin}</td>
                          <td>{hi.porcentajeCobro}</td>
                          <td>{hi.entregables}</td>
                          <td className="borderless-cell"></td>
                        </tr>
                      </>
                    ))}
                  ...
                  <tr>
                    ...
                    <td className="borderless-cell">
                      <div className="pt-3 text-center">Sumatoria:</div>
                    </td>
                    <td>
                      <div className="pt-3 text-end">{this.state.sum} %</div>
                    </td>
                    <td className="borderless-cell">
                      <button
          ...
          <div className="row mx-0 mb-2 mt-4">
            <div className="row fixed-bottom">
              ...
              <button
                className="col-2 offset-1 btn btn-success mb-1"
                onClick={this.sendWorkCalendar}
              >
                Enviar
              </button>
            </div>
