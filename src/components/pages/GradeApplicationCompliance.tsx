...
export default class GradeApplicationCompliance extends Component<
  Props,
  State
> {
  ...
  retrieveComplianceData() {
    ComplianceNotificationDataService.getInfo(
      ...
      .then((response) => {
        this.setState({
          ...
          correctionDeadline: this.convertDate(
            response.data.fechayHoraEntrega,
            true
          ),
          correctionTime: this.convertDate(
            response.data.fechayHoraEntrega,
            false
          ),
  ...
  async postGrade() {
    ...
    await ChangeOrderDataService.createComplianceNotification(
      elem,
      localStorage.getItem("idPostulacionCalificacion") + ""
    )
      .then(() => {})
  ...
  checkScores() {
    for (let i = 0; i < this.state.refScores?.length; i++) {
      if (
        this.state.scoresObtained[i].puntuacion >
          this.state.refScores[i]?.puntaje ||
        !this.state.scoresObtained[i].puntuacion ||
        this.state.scoresObtained[i].puntuacion < 0
      ) {
        this.setState({
          scoresMessage:
            "Tabla de puntajes: Alguno de los puntajes es incorrecto",
        });
        return;
      }
    }
    ...
    return (
      <>
            ...
            <div className="modal-content">
              <div className="modal-header">
                <h5 className="modal-title">
                  ¿Está seguro de cancelar la el registro de su notificacion de
                  conformidad
                </h5>
                <button
                  type="button"
        ...
        <div className="container p-3 position-relative">
          ...
          <div className="form-group row m-3">
            <ScoresTable
              parentCallback={this.handleScores}
              refScores={this.state.refScores}
            />
          </div>
          <div className="form-group row m-3">
            <h5>
              <strong>Informacion de firma de contrato</strong>
            </h5>
          </div>
          <div className="form-group row m-3">
            <label htmlFor="fechaContrato" className="col-md-4 col-form-label">
            ...
            <label
              ...
              className="col-md-1 offset-1 col-form-label"
            >
              a horas
            </label>
            <div className="col-md-2">
